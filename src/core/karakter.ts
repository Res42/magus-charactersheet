...
import { Faj, isFaj } from './components/faj';
import { ISzintlepes, Szintlepes } from './szintlepes';
import { ITulajdonsagok, MAX_TULAJDONSAG } from './components/tulajdonsag';
...
export class Karakter {
  ...
  constructor(public nev: string, public hatterek: Hatter[], private defaultTulajdonsagok: ITulajdonsagok) {
    if (sum(Object.values(defaultTulajdonsagok)) !== MAX_TULAJDONSAG) {
  ...
  get faj() {
    return this.hatterek.filter(isFaj)[0] ?? Faj.Ember;
  }
